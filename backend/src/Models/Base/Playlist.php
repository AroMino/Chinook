...
abstract class Playlist implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Chinook\\Models\\Map\\PlaylistTableMap';
    ...
    /**
     * Initializes internal state of Chinook\Models\Base\Playlist object.
     */
    ...
    public function setPlaylistId($v)
    {
        ...
        if ($this->playlist_id !== $v) {
            $this->playlist_id = $v;
            $this->modifiedColumns[PlaylistTableMap::COL_PLAYLIST_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[PlaylistTableMap::COL_NAME] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : PlaylistTableMap::translateFieldName('PlaylistId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->playlist_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PlaylistTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = PlaylistTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Chinook\\Models\\Playlist'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if (null === $this->playlist_id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('playlist_playlist_id_seq')");
        ...
        if ($this->isColumnModified(PlaylistTableMap::COL_PLAYLIST_ID)) {
            $modifiedColumns[':p' . $index++]  = 'playlist_id';
        }
        if ($this->isColumnModified(PlaylistTableMap::COL_NAME)) {
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(PlaylistTableMap::COL_PLAYLIST_ID)) {
            $criteria->add(PlaylistTableMap::COL_PLAYLIST_ID, $this->playlist_id);
        }
        if ($this->isColumnModified(PlaylistTableMap::COL_NAME)) {
            $criteria->add(PlaylistTableMap::COL_NAME, $this->name);
        }
     ...
     * objects.
     *
     * @param object $copyObj An object of \Chinook\Models\Playlist (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Chinook\Models\Playlist Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
     ...
     * @phpstan-return ObjectCollection&\Traversable<ChildPlaylistTrack}> List of ChildPlaylistTrack objects
     */
    public function getPlaylistTracksJoinTrack(?Criteria $criteria = null, ?ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildPlaylistTrackQuery::create(null, $criteria);
